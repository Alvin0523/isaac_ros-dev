...
if [ "$(docker ps -a --quiet --filter status=running --filter name=$CONTAINER_NAME)" ]; then
    ...
    docker exec -i -t --workdir $ISAAC_ROS_WS $CONTAINER_NAME /bin/bash $@
    exit 0
fi
...
# Set your workspace directory on the host (change as needed)
ISAAC_ROS_DEV_DIR="$HOME/workspaces/isaac_ros-dev"
DOCKER_IMAGE="isaac-ros-htx:latest"
